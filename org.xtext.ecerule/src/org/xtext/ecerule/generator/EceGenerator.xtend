...
class EceGenerator implements IGenerator {
	...
	def compileEcContext(EcContext ecContext, Statement statement) {
		...
		«««»»«compileFluent(ecContext.fluent.get(0), statement)»	«««gestisci il fluente
		«ENDIF»
...
//************************************************************************************************************	
	...
	def compileFluent(Fluent fluent, Statement statement) {
		'''
		ecContext.setFluentName("«fluent.name»");
		
		«compileExpr(fluent.valuePart as ExpressionImpl,  statement)»
		ecContext.setValueExpr(exprContainer);
...
«««		«ENDIF»
	...
	def compileExpr (ExpressionImpl valueExpr, Statement stetement){
			...
			EqualityImpl:{
				...
				«IF cond.op.equals("==")»					««««==
					...
					«ELSE»
						...
						«ENDIF»
						«IF !cond.left.eClass.name.contains("Constant")»
							condContainer = new SameDescr(«compileRecExpr(cond.left as ExpressionImpl, statement)»,«compileRecExpr(cond.right as ExpressionImpl, statement)»);
						«ENDIF»
					«ENDIF»
				expContext.set«condType»Condition(condContainer);
...
//*****COMPILE RECURSIVE EXPRESSION*********************************************************************************
	...
	def dispatch compileRecExpr (EqualityImpl conditionExpr, Statement statement){
		...
		«IF conditionExpr.op.equals("==")»						««««==
			...
			«ELSE»
				«IF conditionExpr.left.eClass.name.contains("Constant")»
					new SameDescr(«compileTerminalLeft(conditionExpr.left as ExpressionImpl, statement)», «compileRecExpr(conditionExpr.right as ExpressionImpl, statement)»)
				«ENDIF»
				«IF !conditionExpr.left.eClass.name.contains("Constant")»
					new SameDescr(«compileRecExpr(conditionExpr.left as ExpressionImpl, statement)»,«compileRecExpr(conditionExpr.right as ExpressionImpl, statement)»)«
				»«ENDIF»
			«ENDIF»
